<?php
   ...
   */
      ...
      # PHP gives a warning if calling count on a scalar, so only use count
      # after checking that $x and $y are not scalars.
      $using_scalars = is_scalar($x) && is_scalar($y);
      if (!$using_scalars && (count($x) != count($y))) {
