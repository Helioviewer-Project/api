...
class Database_Statistics {
    ...
    public function logAccess($action, $ip, $url) {
        $sql = sprintf(
		...
		.     "url"	   . " = '%s';",
		$this->_dbConnection->link->real_escape_string($action),
		$this->_dbConnection->link->real_escape_string($ip),
		$this->_dbConnection->link->real_escape_string($url)
               );
        try {
            $result = $this->_dbConnection->query($sql);
        }
	...
	$dataSourceNames = $this->_getDataSourceStrings();
        ...
        for ($i = 0; $i < $interval["numSteps"]; $i++) {
			...
			//redis table statistics gathering
            $sql = sprintf(
		...
		//additional real-time redis stats
			$statisticsKeys = $redis->keys(HV_REDIS_STATS_PREFIX . "/*");
			$statisticsData = array();
			...
			if($realTimeRedisCount > 0){
				if(isset($counts['totalRequests'][$i][$dateIndex])){
